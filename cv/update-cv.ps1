# Recompile the CV; the single output copy lives at ..\files\cv.pdf (served by the website).
...
Copy-Item "cv_build.pdf" "..\files\cv.pdf" -Force
Remove-Item cv_build.pdf, cv_build.aux, cv_build.log, cv_build.out -Force -ErrorAction SilentlyContinue
Write-Host "CV updated at files\cv.pdf. Now commit and push to publish."
